...
### v0.10.3 — 2026-08-07 · 生图提示词精简去重与排版标签预览

- **动机**：最终生图提示词存在指令文本重复冗长（多段文案被引用多遍、防分隔符警告重复），且"整段必须全英文"等固定规则与中文文案 / 中文排版说明的业务现实冲突；同时预置排版标签仅有文字描述，用户难以直观感知各排版方案的视觉形态。
- **改动点**：
  - **后端 / `prompt_compiler.py`**：多段文案改为编号单次枚举 + 垂直顺序约束，移除重复防分隔符警告与中文空间方位叠加块；删除"完整海报"固定句与结尾全英文 CRITICAL 规则；移除风格 / 排版参考图职责说明行（两类参考图只走 LLM 分析通道，不进入生图请求）；防泄漏提示精简为一句。
  - **后端 / 测试**：新增 `test_prompt_compiler.py`（7 个单测），覆盖编号枚举唯一性、旧重复块移除、单段 verbatim、防泄漏守卫与参考图职责行收敛。
  - **前端 / `LayoutTagPopover`**：六种预置排版标签增加纯 CSS 骨架预览色块（`renderPreviewBlock`），选项行改为左文右图布局，浮层宽度 320→360px。
- **取舍**：删除"完整海报"规则后不再显式约束"成品而非背景图"，空背景风险留待实测观察；防泄漏职责由"只印 Main Title copy"一句承担，若仍出现说明文字被渲染再考虑结构化定界。
...
- **改动点**：
  - **前端 / 历史记录抽屉拆分**：新建独立组件 `SessionHistoryDrawer.tsx`，将会话历史检索、管理模式多选、全选及批量与单条删除逻辑和其相应的 `sessionsList`, `isEditMode`, `selectedIds` 状态完全内嵌其中，且只在打开时按需触发列表加载。
  - **前端 / 工作区结构精简**：从 `AgentWorkspace.tsx` 中移除了所有内嵌的历史会话管理逻辑与状态，移除了内联的 `<style>` 标签样式注入，引入 `<SessionHistoryDrawer>` 进行渲染，组件行数精简近 300 行。
  - **前端 / 禁用类全局样式化**：将出图/确认执行状态下禁用的 `.generating-disabled` 与 `.confirmed-disabled` 交互禁用类定义移至全局 `index.css`。
- **取舍**：
  - **组件单向状态流动**：由主工作区作为 `useAgentSession()` 状态 hook 的唯一管理者，通过 Props 单向注入会话操作函数，防止子组件重复声明 state 实例。

### v0.10.0 — 2026-08-05 · 会话记录管理与标注/加载界面体验升级

- **动机**：打通会话历史清理链路，解决 AI 设计助理模式下无用的新会话堆积且无法整理的问题；优化海报编辑工具栏与图片生成加载界面的视觉质感，防止 9:16/9:32 等窄高画幅下的布局畸形与参数截断。
- **改动点**：
  - **后端 / 会话批量删除**：在 `skill_runner.py` 增加 `delete_sessions` 批量软删除，暴露 `POST /api/agent/sessions/batch-delete` 端点，增加 `test_batch_delete.py` 单元测试。
  - **前端 / 会话抽屉管理**：重构 `AgentWorkspace.tsx` 会话抽屉，新增“管理模式”切换、复选框多选、全选及底部批量操作栏，处理当前活跃会话被删时的优雅切换与重建。
  - **前端 / 标注工具栏线稿化**：将工具图标由 Emoji（🖊/➡/▭/◯/🧹）替换为 2.5px SVG 矢量线稿，实现 Flat UI 无边框设计，按钮 Hover Notion 灰，Active 淡红遮罩。
  - **前端 / 生成加载界面简约化**：重构 `status-views.tsx` 的 `BlueprintLoading` 视图，移除冗余大占位框，采用中心居中弹性布局，用 Sparkles 图标代替 Emoji 🎨 并压缩底栏参数，解决比例畸形和截断。
  - **前端 / 冗余垃圾清理与 Bug 修复**：删除素材弹出菜单底部的冗余文本列表（清理 `AgentWorkspace.tsx` 与 `UploadBar.tsx`）；修复 9:32 画布贴顶导致等待状态文案不居中的 Bug。
- **取舍**：
  - **历史记录软删除**：会话删除物理上不删库（更新 `deleted_at` 字段），与现有 generations 归档 GC 兼容。

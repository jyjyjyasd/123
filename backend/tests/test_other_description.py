...
from app.agent.prompt_compiler import compile_prompt

@pytest.mark.asyncio
async def test_other_image_uses_user_description_not_vision_llm(
    client: AsyncClient, session, async_session
) -> None:
    # 1️⃣ 登录
    ...
    # 3️⃣ 手动把该文件作为 "other" 材料加入会话，带 description
    async with async_session() as db:
        # 获取已有的会话对象（fixture `session` 为已有 Session 实例）
        s = await db.get(type(session), session.id)
        stream_b = {
            "subject_materials": [
                {
                    "id": "mat-1",
    ...
    # 4️⃣ 验证 compile_prompt 使用用户提供的 description，并且 Vision LLM 未被调用
    with patch("app.agent.llm_client.describe_reference_image") as mock_desc:
        prompt = await compile_prompt(s, db=db)

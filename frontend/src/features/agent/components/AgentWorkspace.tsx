...
export function AgentWorkspace() {
  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      ...
      {showSessionHistory && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 1000, display: "flex", justifyContent: "flex-end" }} onClick={() => setShowSessionHistory(false)}>
          <div style={{ width: 320, height: "100%", background: "#fff", display: "flex", flexDirection: "column", padding: 16 }} onClick={(e) => e.stopPropagation()}>
            <div style={{ borderBottom: "1px solid rgba(55,53,47,0.09)", paddingBottom: 16, display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontWeight: 700 }}>AI 设计助理会话记录</span>
              <button onClick={() => setShowSessionHistory(false)} style={{ border: "none", background: "transparent", cursor: "pointer" }}>✕</button>
            </div>
            <div style={{ flex: 1, overflowY: "auto", paddingTop: 16 }}>
              {sessionsList.map((item) => (
                <div key={item.id} onClick={() => { loadSession(item.id); setShowSessionHistory(false); }} style={{ padding: "10px", borderRadius: 8, border: "1px solid rgba(55,53,47,0.08)", marginBottom: 8, cursor: "pointer" }}>
                  <div style={{ fontWeight: 600 }}>{item.subject_description || "新会话"}</div>
                  <div style={{ fontSize: 11, color: "#9b9a97", marginTop: 4 }}>{new Date(item.created_at).toLocaleString()}</div>
                </div>
              ))}
            </div>
          </div>
